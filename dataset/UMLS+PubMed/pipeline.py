...
create_neg(os.path.join(NEW_DATASET, 'test.txt'), os.path.join(NEW_DATASET, 'test_neg.txt'), neg_ratio=30)
create_neg(os.path.join(NEW_DATASET, 'valid.txt'), os.path.join(NEW_DATASET, 'valid_neg.txt'), neg_ratio=30)
...
lines = open('./UMLS+PubMed.tucker.256.kge_neg.txt').readlines()
random.shuffle(lines)
...
lines = open('UMLS+PubMed.tucker.256.test.link_prediction.txt').readlines()
